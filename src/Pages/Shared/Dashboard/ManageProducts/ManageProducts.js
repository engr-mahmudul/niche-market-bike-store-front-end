...
const ManageProducts = () => {
    ...
    useEffect(() => {
        const url = 'https://arcane-escarpment-94457.herokuapp.com/products'
        // console.log(url);
        fetch(url)
            .then(res => res.json())
    ...
    const deleteHandle = id => {
        ...
        if (procced) {
            const url = `https://arcane-escarpment-94457.herokuapp.com/products/${id}`;
            fetch(url, {
                method: "DELETE"
            })
                .then(res => res.json())
    ...
    return (
        <div>
            <Typography variant='h5' sx={{ fontWeight: 700, mb: 3, color: 'green', textAlign: 'center' }}>
                Our Shop's products Total : {products.length}
            </Typography>
                ...
                <Table sx={{}} aria-label="simple table">
                    <TableHead sx={{ backgroundColor: '#08a69e' }}>
                        <TableRow>
                            ...
                            <TableCell sx={{ fontWeight: 700, color: 'white' }} align="center">Version</TableCell>
                    ...
                    <TableBody>
                        {products.map((row) => (
                            ...
                            >
                                ...
                                <TableCell align="center" sx={{ color: 'green' }}>{row?.price}</TableCell>
